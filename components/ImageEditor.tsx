      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="font-display font-bold text-4xl md:text-6xl text-white mb-6 tracking-tight">
            AI CREATIVE <span className="text-gold-gradient">STUDIO</span>
          </h2>
        ...
        <div className="bg-black-card border border-white/5 rounded-sm p-6 md:p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 relative z-10">
            ...
            <div className="flex flex-col space-y-6">
              <div 
                className={`group relative w-full h-80 md:h-96 rounded-sm border border-dashed transition-all duration-300 flex flex-col items-center justify-center bg-black-lighter overflow-hidden cursor-pointer ${selectedImage ? 'border-gold/50 bg-black-lighter/80' : 'border-white/10 hover:border-gold/40 hover:bg-white/5'}`}
              ...
              >
                {selectedImage ? (
                  ...
                  <div className="text-center p-6 transition-transform group-hover:scale-105 duration-300">
                    <svg className="mx-auto h-12 w-12 text-gold opacity-80" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex flex-col space-y-2">
                <label className="text-xs font-bold text-gold uppercase tracking-widest">Instructions</label>
                <textarea
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  placeholder='e.g., "Add a golden aura", "Make it look like a vintage sports card", "Remove the background"'
                  className="w-full bg-black-lighter text-white p-4 rounded-sm border border-white/10 focus:border-gold focus:ring-1 focus:ring-gold/50 focus:outline-none resize-none h-28 placeholder-gray-600 transition-colors"
                />
              </div>
            ...
            <div className="flex flex-col h-full">
               <div className="text-xs font-bold text-gold uppercase tracking-widest mb-2 lg:hidden">Result</div>
               <div className={`relative w-full h-80 md:h-[calc(100%-2rem)] rounded-sm border border-white/10 bg-black-rich flex items-center justify-center overflow-hidden shadow-inner`}>
                 {isLoading ? (
                    <div className="text-center">
                       <div className="inline-block animate-pulse-slow w-20 h-20 rounded-full border-2 border-gold/30 mb-4 flex items-center justify-center">
                         <div className="w-16 h-16 bg-gold/10 rounded-full animate-ping"></div>
                       </div>
                       <p className="text-gold text-sm font-medium tracking-wide animate-pulse">AI IS CREATING...</p>
                    </div>
                 ) : generatedImage ? (
               ...
               {generatedImage && (
                 <div className="mt-4">
                    <a 
                      ...
                      className="block w-full text-center py-3 border border-gold/30 rounded-sm hover:bg-gold/10 text-gold transition-colors font-medium uppercase tracking-wider text-sm"
                    >
                      Download Image
                    </a>
                 </div>
